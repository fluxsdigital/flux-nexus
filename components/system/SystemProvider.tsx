...
type Store = { companies:Company[]; equipment:Equipment[]; inspections:Inspection[]; companyFilter:string; setCompanyFilter:(v:string)=>void; addCompany:(v:Omit<Company,"id">)=>Promise<void>; addEquipment:(v:Omit<Equipment,"id">)=>Promise<void>; addInspection:(v:Omit<Inspection,"id">)=>Promise<string> };
...
export function SystemProvider({children}:{children:React.ReactNode}){
  ...
  const addInspection=async(value:Omit<Inspection,"id">)=>{const i=await request<ApiInspection>("/inspections",{method:"POST",body:JSON.stringify({equipmentId:value.equipmentId,type:inspectionTypeToApi[value.type]??"PERIODIC_EXTERNAL",scheduledAt:new Date().toISOString()})});setInspections(items=>[...items,{id:i.id,equipmentId:i.equipmentId,date:formatDate(i.scheduledAt),type:i.type,status:inspectionStatusFromApi[i.status]??i.status,result:i.result?inspectionResultFromApi[i.result]??i.result:undefined}]);return i.id};
