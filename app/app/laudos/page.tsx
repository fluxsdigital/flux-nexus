...
export default function Reports(){
  ...
  const document=selectedReport&&<article className="document technicalReport">
    ...
    {data?<>
      ...
      {data.evidence.length>0&&<ReportSection title="9. Evidências e anexos"><ol className="reportEvidence">{data.evidence.map((evidence,index)=><li key={index}><b>{evidence.title}</b><span>{evidence.description}</span><small>{evidence.url}</small></li>)}</ol></ReportSection>}
