...
export default function Reports(){
  ...
  const emit=async()=>{
    ...
    try{
      ...
      setReports(items=>[report,...items]);setSelectedReport(report);setSelectedInspection(null);
  ...
  const document=selectedReport&&<article className="document technicalReport">
    ...
    {data?<>
      ...
      {data.evidence.length>0&&<ReportSection title="9. Evidências fotográficas"><div className="reportEvidenceGrid">{data.evidence.map((evidence,index)=><figure key={evidence.id||index}>{(evidence.dataUrl||evidence.url)&&<img src={evidence.dataUrl||evidence.url} alt={evidence.title}/>}<figcaption><b>{evidence.title}</b><span>{evidence.description}</span><small>{evidence.fileName}{evidence.sha256?` · SHA-256 ${evidence.sha256.slice(0,16)}…`:""}</small></figcaption></figure>)}</div></ReportSection>}
